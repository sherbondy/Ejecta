...
static inline EJColorRGBA EJCanvasBlendColor( EJCanvasState *state, EJColorRGBA color ) {
	...
	return (EJColorRGBA){ .rgba = {
		.r = (u_char)((float)color.rgba.r * alpha),
		.g = (u_char)((float)color.rgba.g * alpha),
		.b = (u_char)((float)color.rgba.b * alpha),
		.a = (u_char)(EJCompositeOperationFuncs[state->globalCompositeOperation].alphaFactor *
